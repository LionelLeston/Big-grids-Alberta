heth<-read.csv("BG-HETH-occupancy.csv")
str(heth)
#assign round
heth$round <- ave(heth$HETH, heth$Station, FUN = seq_along)

names = c("HETH")
names   ## You could use the following loop if you 
#have abundance of different species in different columns 
#or you could replace "spp" with heth$HETH in the tapply function

for(i in names){ 
  spp<-heth[,c(i)]  #Species as variable
  
  recasted<-tapply(spp, list(heth$Station, heth$round), max, na.rm=TRUE)
  recasted.df<-data.frame(recasted)
  recasted.df$Site<-row.names(recasted.df)
  recasted.df$D1<-recasted.df$X1
  recasted.df$D2<-recasted.df$X2
  recasted.df$D3<-recasted.df$X3
  recasted.df$D4<-recasted.df$X4
  #I'm assuming that you're not interested in a round 5 or 6
  recasted.final<-recasted.df[,c("Site","D1","D2","D3","D4")]
  write.csv(recasted.final, paste0("recast.",i,".csv"))
}

#You would do the same thing for your variable "Julian" but create a 
#separate data frame with columns Site, J1, J2, J3, and J4
#Then merge the two data frames using the column Site
...
#names = colnames(merge2.yearround[,c(11:165)]) 
names = c("ALFL","AMRE","AMRO","BBWA","BCCH","BOCH","BRCR","BTNW","CAWA","CMWA","CONW","DEJU","GCKI","GRAJ","HAWO","HETH","LEFL","LISP","MAWA","MOWA","OVEN","RBGR","RBNU","RCKI","REVI","RUGR","SWSP","SWTH","TEWA","WAVI","WETA","WIWR","WTSP","YBSA","YEWA","YRWA")
